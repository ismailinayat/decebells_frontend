...
const fetchProducts = async () => {
  const {data} = await axios.get('http://localhost:5000/api/v1/products?mainCategory=Wired');
  return data;
}
...
function Wired(props) {
  ...
  const {data, status} = useQuery('wiredProducts', fetchProducts, {initialData: props.products});
